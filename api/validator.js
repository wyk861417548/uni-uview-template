  ...
  IsDate : function(op, formatString){
	...
	validate(formdata,rule){
       ...
       // 如果没填信息
        ...
        console.log("check",rule[i],type);
				uni.showToast({title:rule[i].msg ? rule[i].msg : "信息有误，请检查填写信息。",icon:"none"})
				check = false;
				break;
			}
			...
			if(this[type] && type != "IdCard"){
				check = this[type].test(value);
			}else{
				//额外扩展
				switch (type){
					case "IdCard":
						check = this.IdCard(value);
						break;
				}
			}
        ...
        uni.showToast({title:rule[i].errMsg ? rule[i].errMsg : "信息有误，请检查填写信息。",icon:"none"})
				break;
			}
		}
